 ...
 */
add_filter( 'template_include', 'b4l_front_end_user_profile_template');
 ...
 */
function b4l_front_end_user_profile_template( $template ) {
    //checking if the page has the slug of accept-badge
    if ( is_page( 'user-profile' )  ) {
        $file = WP_PLUGIN_DIR . '/badges4languages-plugin/templates/single_front_end_user_profile.php';
        if (file_exists($file)) {
            $template = $file;
        }
    }
